...
-(void)addProgressViewWithIndex:(NSInteger)index percent:(CGFloat)percent{
    ...
    progressBgView.backgroundColor = RGBHex(0xf0f1f2);//  [UIColor colorWithHexString:@"#f0f1f2" alpha:1.f];
...
-(UIColor *)getColorWithProgressIndex:(NSInteger)index{
    UIColor *color = RGBHex(0xff643d);//[UIColor colorWithHexString:@"#ff643d" alpha:1.f];
    if([self.resultDic[@"correctOption"] isKindOfClass:[NSNumber class]]) {//如果是单选
        if ([self.resultDic[@"correctOption"] integerValue] == index - 1) {//判断此条数据是否是正确答案
            return  RGBHex(0x17bc2f);//[UIColor colorWithHexString:@"#17bc2f" alpha:1.f];//返回正确答案的颜色
        }
    } else {
...
-(UIView *)labelBgView {
    if(!_labelBgView) {
        ...
        _labelBgView.backgroundColor = RGBHex(0xffffff);// [UIColor colorWithHexString:@"#ffffff" alpha:1.f];
        _labelBgView.layer.masksToBounds = YES;
        _labelBgView.layer.cornerRadius = CCGetRealFromPt(20);
        _labelBgView.layer.borderColor =  RGBHex(0xdddddd).CGColor;//[UIColor colorWithHexString:@"#dddddd" alpha:1.f].CGColor;
...
-(UILabel *)titleLabel{
    if (!_titleLabel) {
        ...
        _titleLabel.textColor = RGBHex(0x1e1f21);//[UIColor colorWithHexString:@"#1e1f21" alpha:1.f];
