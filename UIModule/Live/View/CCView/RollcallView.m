...
//    NSLog(@"_duration = %d",(int)_duration);
    if(_duration == 0) {//签到时间为零时,设置视图样式
        ...
        self.label.textColor = RGBHex(0xff412e);//[UIColor colorWithHexString:@"#ff412e" alpha:1.f];
...
-(UILabel *)label {
    if(!_label) {
        ...
        _label.textColor = RGBHex(0x1e1f21);//[UIColor colorWithHexString:@"#1e1f21" alpha:1.f];
