         ...
         selectButton:(UIButton *)selectButton
{
    ...
    if (self.isStastic) {
        ...
        bgView.backgroundColor = RGBHex(0xedf5ff);// [UIColor colorWithHexString:@"#edf5ff" alpha:1.0f];
        ...
        dataView.backgroundColor = RGBHex(0x43b50f);//[UIColor colorWithHexString:@"#43b50f" alpha:1.0f];
...
//    selectButton.userid = buttonId;
    selectButton.accessibilityIdentifier = buttonId;
...
//    uiTextView.userid = opthionsDic[@"id"];
     uiTextView.accessibilityIdentifier =  [NSString stringWithFormat:@"%@",opthionsDic[@"id"]];
...
-(void)addTitle{
    ...
    contentLabel.textColor = RGBHex(0x1e1f21) ;//[UIColor colorWithHexString:@"#1e1f21" alpha:1.f];
    //设置问卷标题约束
...
-(void)submitAction {
    ...
    for(UIButton *button in self.buttonArray) {
        if(button.selected == YES) {
            NSArray *separatedArray = [button.accessibilityIdentifier componentsSeparatedByString:@"-"];//[button.userid componentsSeparatedByString:@"-"];
    ...
    for(UITextView *textView in self.textViewArray) {
        NSString *subjectId =textView.accessibilityIdentifier ;//textView.userid;
        BOOL flag = NO;
        NSMutableDictionary *dicGet = nil;
        for(NSMutableDictionary *dic in array) {
            if([dic[@"subjectId"] isEqualToString:subjectId]) {
...
-(void)selectBtnClicked:(UIButton *)sender {
    ...
    sender.selected = YES;
    _msgLabel.hidden = YES;
    NSString *str = sender.accessibilityIdentifier;// sender.userid;
    ...
    if([strArr[1] intValue] == 0) {
        for(int i = 0;i < [strArr[2] intValue]; i++) {
            ...
            if(![userId isEqualToString:[self separatedString:sender.accessibilityIdentifier]]) {
                for(UIButton *button in self.buttonArray) {
                    if([userId isEqualToString:[self separatedString:button.accessibilityIdentifier]]) {
                        button.selected = NO;
                    }
                }
            }
...
-(UILabel *)titleLabel {
    if(!_titleLabel) {
        ...
        _titleLabel.textColor = RGBHex(0x38404b) ;//[UIColor colorWithHexString:@"#38404b" alpha:1.f];
