...
@implementation NSURLSession (BugRepairResumeData)
...
- (NSURLSessionDownloadTask *)downloadTask_ios10_1_WithResumeData:(NSData *)resumeData {
    ...
    NSData *cData = BUGcorrectResumeData(resumeData);
    cData = cData?cData:resumeData;
    NSURLSessionDownloadTask *task = [self downloadTaskWithResumeData:cData];
    NSMutableDictionary *resumeDic = BUGgetResumeDictionary(cData);
    if (resumeDic) {
...
NSData * BUGcorrectRequestData(NSData *data) {
    if (!data) {
        return nil;
    }
...
NSMutableDictionary *BUGgetResumeDictionary(NSData *data) {
...
NSData *BUGcorrectResumeData(NSData *data) {
    NSString *kResumeCurrentRequest = @"NSURLSessionResumeCurrentRequest";
    NSString *kResumeOriginalRequest = @"NSURLSessionResumeOriginalRequest";
    if (data == nil) {
        return  nil;
    }
    NSMutableDictionary *resumeDictionary = BUGgetResumeDictionary(data);
    if (resumeDictionary == nil) {
        return nil;
    }
    resumeDictionary[kResumeCurrentRequest] = BUGcorrectRequestData(resumeDictionary[kResumeCurrentRequest]);
    resumeDictionary[kResumeOriginalRequest] = BUGcorrectRequestData(resumeDictionary[kResumeOriginalRequest]);
