...
#import "NSURLSession+BugRepairResumeData.h"
...
// >>>>>>>>>>>>>>>>>>>>>>>>>>  task info
// 下载状态
@property (nonatomic, assign) CCPBDownloadState state;
// 下载任务
        ...
        if ([NSKeyedUnarchiver unarchiveObjectWithFile:savePath]) {
            ...
            if (_downloadModels.count != 0) {
                for (CCDownloadModel * downloadModel in _downloadModels) {
                    if (downloadModel.state == CCPBDownloadStateRunning) {
...
- (void)startWithDownloadModel:(CCDownloadModel *)downloadModel progress:(CCDownloadProgressBlock)progress state:(CCPBDownloadStateBlock)state
{
...
- (void)startWithDownloadModel:(CCDownloadModel *)downloadModel
{
    ...
    if (downloadModel.state == CCPBDownloadStateReadying) {
    ...
    if (task && task.state == NSURLSessionTaskStateRunning) {
        downloadModel.state = CCPBDownloadStateRunning;
        [self downloadModel:downloadModel error:nil];
        return;
    }
...
- (void)suspendWithDownloadModel:(CCDownloadModel *)downloadModel
{
    ...
    downloadModel.state = CCPBDownloadStateSuspended;
    ...
    if (!task || task.state == NSURLSessionTaskStateCanceling) {
        ...
        if ([self isValideResumeData:resumeData]) {
            ...
            if (IOS_SYSTEMVERSION == 10.0 || IOS_SYSTEMVERSION == 10.1) {
                ...
                task = [self.session downloadTask_ios10_1_WithResumeData:resumeData];
            }else{
...
//        NSLog(@"取消，并且删除了数据");
    ...
    if (error && resumeData) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^(){
            downloadModel.state = CCPBDownloadStateFailed;
        ...
        //下载完成
        dispatch_async(dispatch_get_main_queue(), ^(){
            downloadModel.state = CCPBDownloadStateCompleted;
            [self downloadModel:downloadModel error:nil];
...
{
    if (downloadModel.state == CCPBDownloadStateCompleted) {
        return YES;
    }
    return NO;
}
...
- (BOOL)canResumeDownlaodModel:(CCDownloadModel *)downloadModel
{
    ...
    @synchronized (self) {
        ...
        if (downloadingCount >= _maxDownloadCount) {
            downloadModel.state = CCPBDownloadStateReadying;
            [self downloadModel:downloadModel error:nil];
    ...
    @synchronized (self) {
        ...
        if (_resumeDownloadFIFO) {
            ...
            [self.downloadModels enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                if (((CCDownloadModel *)obj).state == CCPBDownloadStateReadying) {
...
//查询所有已下载完成，未下载完成的
- (NSArray *)queryAllDownloaded:(BOOL)downloaded {
    ...
    for (CCDownloadModel *model  in self.downloadModelList) {
        if (model.state == 4) {
            ...
            [loading addObject:model];
        }
    }
    return downloaded?loaded:loading;
